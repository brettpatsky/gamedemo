...
const _FOG_Z      := 45
# Shared soft-circle texture for rain/snow particles — generated once per
# AmbientLayer instance. Without a texture, CPUParticles2D renders as
# single white pixels that are invisible at the squad's view distance.
var _particle_tex: ImageTexture   = null
...
func _ready() -> void:
	...
	_particle_tex = _make_soft_circle_texture(6)
...
func _refresh_view_extents() -> void:
	...
	_view_half_h = new_h
	if _weather_emit:
		_weather_emit.emission_rect_extents = Vector2(_view_half_w + VIEW_PADDING, 8.0)
		_weather_emit.position = Vector2(0.0, -_view_half_h - VIEW_PADDING * 0.5)
	if _fog_active:
		queue_redraw()

# Tiny soft-circle ImageTexture used as the particle sprite for both rain
# and snow. Without a texture, CPUParticles2D renders as single white
# pixels which are invisible at the player's view distance.
...
func _setup_weather() -> void:
	...
	p.emitting     = true
	p.texture      = _particle_tex   # without this, particles are invisible
	# CPUParticles2D uses the 2D rectangle shape; the 3D _BOX variant lives
	# on GPUParticles3D and uses a Vector3 extent. Size derived from the
	# actual viewport (see _refresh_view_extents).
	p.emission_shape         = CPUParticles2D.EMISSION_SHAPE_RECTANGLE
	p.emission_rect_extents  = Vector2(_view_half_w + VIEW_PADDING, 8.0)
	p.position = Vector2(0.0, -_view_half_h - VIEW_PADDING * 0.5)
	...
	if weather == Weather.RAIN:
		...
		# Scale stretches the soft-circle texture (13 px) — vertical-favouring
		# scale would need a custom curve; uniform scale is fine for "rain".
		p.scale_amount_min     = 1.2
		p.scale_amount_max     = 2.0
		var ramp := Gradient.new()
		ramp.set_color(0, Color(0.72, 0.83, 1.0, 0.90))
		ramp.set_color(1, Color(0.72, 0.83, 1.0, 0.10))
		p.color_ramp = ramp
	else:   # SNOW
		p.amount       = 160
		p.lifetime     = 5.0
		...
		ramp.set_color(1, Color(1.0, 1.0, 1.0, 0.55))
		p.color_ramp = ramp
	add_child(p)
	_weather_emit = p
