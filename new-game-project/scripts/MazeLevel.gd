...
# open cells of the maze, far from the spawn corner so the player has to
# actually move through the maze to reach them. Same `child_slot` /
# fragment_id contract as MapGenerator._spawn_mission_parent_and_fragment.
func _spawn_mission_parent_and_fragment() -> void:
	...
	for row_idx in MAZE_LAYOUT.size():
		...
		for col_idx in row.length():
			if row[col_idx] != ".":
				continue
			# Spawn is at (1,1); skip a 4-cell halo so neither object lands
			# on top of the squad's starting position.
	...
	if _MEMORY_FRAGMENT_SCENE:
		# Fragment goes in the cell farthest from the cage so they're not
